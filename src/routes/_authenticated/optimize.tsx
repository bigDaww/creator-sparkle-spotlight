...
// ---------- Pre-Publish Score tab ----------

type PPChapter = { timestamp: string; label: string };
type PPComponentKey = "title" | "description" | "chapters" | "thumbnail_text";
type PPBreakdown = Record<PPComponentKey, { score: number; verdict: string; fixes: string[] }>;
type PPResult = {
...
const PP_LABELS: Record<PPComponentKey, string> = {
  title: "Title",
...
function PrepublishTab() {
  ...
  const [chapters, setChapters] = useState<PPChapter[]>([{ timestamp: "0:00", label: "" }]);
  const [result, setResult] = useState<PPResult | null>(null);
  const [locked, setLocked] = useState(false);
  ...
  const historyLocked =
    historyQ.error instanceof Error && historyQ.error.message.includes("PAID_PLAN_REQUIRED");
  const history = (historyQ.data ?? []) as PPHistoryRow[];
  ...
  const mut = useMutation({
    ...
    onSuccess: (r) => {
      setLocked(false);
      try {
        const parsed = JSON.parse((r as { result: string }).result) as PPResult;
        setResult(parsed);
  ...
  return (
    <div className="space-y-8">
      <form
        onSubmit={(e) => {
          ...
          if (!title.trim() && !description.trim())
            return toast.error("Add at least a title or description");
          setResult(null);
          setLocked(false);
          mut.mutate();
        }}
      ...
      {locked && (
        <UpgradeGate
          title="Pre-Publish Check is a Pro feature"
          body="Upgrade to score your full upload package for AI answer engines before you hit publish, with fixes for every weak component."
        />
      )}

      {result && <PPResultView result={result} />}

      {!historyLocked && <PPHistorySection rows={history} loading={historyQ.isLoading} />}
    </div>
  );
}

function PPResultView({ result }: { result: PPResult }) {
  const composite = Math.max(0, Math.min(100, Math.round(result.composite_score ?? 0)));
  const components: PPComponentKey[] = ["title", "description", "chapters", "thumbnail_text"];
  return (
          ...
          {components.map((k) => {
            ...
            return (
              <div key={k} className="rounded-2xl border border-border/60 bg-background/40 p-5">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold">{PP_LABELS[k]}</p>
...
function PPHistorySection({ rows, loading }: { rows: PPHistoryRow[]; loading: boolean }) {
  if (loading) {
    return (
      <section>
